...
class TestConfigWorkflows:  # pylint: disable=too-few-public-methods
    ...
    def test_circular_workflow(  # pylint: disable=too-many-locals  # noqa: PLR0914,PLR6301
        ...
        loaded_running_text = "\n".join(
            line.render() for line in running_config.all_children_sorted()
        )
        ...
        loaded_generated_text = "\n".join(
            line.render() for line in generated_config.all_children_sorted()
        )
        ...
        remediation_text = "\n".join(
            line.render() for line in remediation_config.all_children_sorted()
        )
        ...
        future_lines = {
            ...
            if not line.render().strip().startswith(("no ", "delete "))
        }
        generated_lines = {
            line.render()
        ...
        rollback_text = "\n".join(
            line.render() for line in rollback_config.all_children_sorted()
        )
